...
class WidgetSnapshotterStateImpl extends WidgetSnapshotterState {
  ...
  @override
  Future<TargetedImage?> getSnapshot(
    Offset location,
    Object key,
    ValueGetter<Widget?> widgetBuilder,
  ) async {
    if (_registeredWidgets.containsKey(key)) {
      return _getSnapshotInner(location, key);
    } else {
      registerWidget(key, widgetBuilder.call());
      try {
        return await _getSnapshotInner(location, key);
      } finally {
  ...
  Future<TargetedImage?> _getSnapshotInner(
    Offset location,
    Object key,
  ) {
...
Future<TargetedImage> _getSnapshot(
  ...
  final image = renderObject.toImageSync(pixelRatio: devicePixelRatio);
