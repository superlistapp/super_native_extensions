...
class DefaultMenuTheme {
  ...
  final TextStyle Function(MenuItemInfo) textStyleForItem;
  final BoxDecoration Function(MenuItemInfo) decorationForItem;

  static DefaultMenuTheme themeForBrightness(Brightness brightness) {
    switch (brightness) {
      case Brightness.light:
        return DefaultMenuTheme(
          ...
          inactiveMenuVeilColor: (depth) =>
              Colors.grey.withOpacity((depth * 0.5).clamp(0.0, 0.8)),
      ...
      case Brightness.dark:
        return DefaultMenuTheme(
          ...
          menuDecorationInside: const BoxDecoration(
          ...
          menuPreviewDecorationInside: BoxDecoration(
            color: const Color(0xFF5B5E75).withOpacity(0.5),
          ),
          backgroundTintColor: (hasBlur) => hasBlur
              ? const Color(0xF3333333).withOpacity(0.3)
              : const Color(0xFF333333).withOpacity(0.5),
          separatorColor: const Color(0xFF4C4F63),
          inactiveMenuVeilColor: (depth) => const Color(0xFF4C4F63)
              .withOpacity(((depth * 0.6).clamp(0.0, 0.8))),
          // Colors.grey.shade700.withOpacity((depth * 0.5).clamp(0.0, 0.8)),
          textStyleForItem: (info) => TextStyle(
            color: info.isDestructive
                ? Colors.red.shade200
                : info.isDisabled
...
class DefaultMenuWidgetBuilder extends MenuWidgetBuilder {
  DefaultMenuWidgetBuilder({
  ...
  DefaultMenuTheme _getTheme(BuildContext context) {
    return DefaultMenuTheme.themeForBrightness(_getBrightness(context));
  }
  ...
  @override
  Widget buildMenu(BuildContext context, MenuInfo menuInfo, Widget child) {
    return child;
  }
...
class _MenuItem extends StatelessWidget {
  ...
  final MenuButtonState state;
  final MenuElement element;
  final DefaultMenuTheme theme;
